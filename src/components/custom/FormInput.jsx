          ...
          <div className="flex flex-wrap gap-1.5 items-center overflow-hidden">
            {value.length > 0 ? (
              value.length > 5 ? (
                ...
                selectedLabels.map(label => (
                  <span key={label} className="bg-blue-50/80 text-blue-700 text-[10px] font-bold px-2 py-0.5 rounded-md border border-blue-100/50">
                    {label}
                  </span>
                ))
              )
...
export const FormInput = ({
    ...
    switch (type) {
      ...
      case "switch":
        ...
        return (
          <div className={cn(
            "flex items-center gap-4 py-3.5 px-5 transition-all duration-300 rounded-2xl border",
            isChecked ? "bg-green-50/20 border-green-100/50 shadow-sm" : "bg-slate-50/50 border-slate-100 shadow-inner"
          )}>
            <Switch
              checked={isChecked}
              onCheckedChange={(checked) => {
            ...
            />
            <div className="flex flex-col">
              {placeholder && <span className="text-sm font-bold text-slate-700 leading-none mb-1">{placeholder}</span>}
              <span className={cn("text-[10px] uppercase font-black tracking-widest", isChecked ? "text-green-600" : "text-amber-600")}>
                {isChecked ? t('active') : t('inactive')}
              </span>
            </div>
          </div>
  ...
  return (
    <div className={cn("space-y-2.5 w-full group", className)} dir={dir}>
      {label && (
        <div className={cn("flex items-center gap-1.5 select-none", isRtl ? 'mr-1' : 'ml-1')}>
          <label className="text-xs font-bold text-slate-500 uppercase tracking-widest group-focus-within:text-blue-600 transition-all">
            {label}
          </label>
        </div>
      )}
      ...
      {hasError && (
        <p className={cn("text-[10px] text-red-500 font-bold flex items-center gap-2", isRtl ? 'mr-1.5' : 'ml-1.5')}>
